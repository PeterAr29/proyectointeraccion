...
- **Reclamar antes de codificar:** marca el módulo/tarea _En progreso_ en el GitHub Project y en `progreso/tablero-equipo.md`.
...
`docs/evaluacion-usabilidad.md`, `tests/e2e/*` (flujos críticos), configuración de headers de seguridad, política de privacidad, y los assets de **PWA** (`app/manifest.ts`, íconos 192/512/maskable + apple-touch-icon, service worker).
...
### SUBFASE 6.2: Endurecimiento, PWA y despliegue

```
Lee CLAUDE.md y progreso/fase-6.1.md. Estamos en F6.2 (última). Endurece, haz PWA y despliega:
- Headers de seguridad (CSP, HSTS, X-Frame-Options, X-Content-Type-Options, Referrer-Policy),
  CORS al dominio de Vercel, cookies seguras. La CSP debe permitir el service worker y el manifest.
- PWA instalable en móviles (RNF-10, especificaciones §9.4): app/manifest.ts (standalone,
  theme/background color, start_url), íconos 192/512 + maskable + apple-touch-icon, meta
  theme-color/viewport, y service worker para instalabilidad y offline básico (cachear el shell;
  los datos siguen en Supabase). Engancha el diálogo global `offline` (F1.3) al estado de conexión.
  Si añades una librería para el SW (p. ej. @ducanh2912/next-pwa), evalúala (§5.3) y registra ADR.
...
- Deploy a Vercel con variables de entorno de producción; verifica el flujo end-to-end y que la
  app sea instalable como PWA en un móvil (Lighthouse PWA en verde).
Commit chore(F6.2), tag v1.0.0, y actualiza estado-actual + ROADMAP a 100%.
```

**DoD:** headers+CORS · **PWA instalable (manifest+SW+íconos, Lighthouse PWA verde)** · e2e críticos verdes · audit limpio · política de privacidad · restore probado · deploy verificado · tag `v1.0.0`.
